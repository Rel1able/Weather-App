...
const temperatureDiv = document.getElementById("temperature") as HTMLDivElement;
const humidityDiv = document.getElementById("humidity") as HTMLDivElement;
const windDiv = document.getElementById("wind") as HTMLDivElement;
const precipitationsDiv = document.getElementById(
  "precipitations",
) as HTMLDivElement;
const weatherData = document.querySelector(".weather-data") as HTMLDivElement;
...
async function fetchWeatherData() {
  try {
    ...
    const response = await fetch(
      `https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/${cityName}?key=KQYWTLZ3T34A25ZMDLD69TJM6`,
      { mode: "cors" },
    );
    if (!response.ok) {
...
form.addEventListener("submit", (e) => {
  e.preventDefault();
  const inputValue = document.getElementById("cityName") as HTMLInputElement;
